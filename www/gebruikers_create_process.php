...
$required_fields = ['voornaam', 'achternaam', 'email', 'role', 'woonplaats', 'postcode', 'huisnummer', 'gebruikersnaam', 'wachtwoord', 'verzeker_wachtwoord'];
...
$stmt = $conn->prepare("SELECT COUNT(*) FROM gebruikers WHERE 
CONCAT(email, '|', voornaam, '|', achternaam, '|', gebruikersnaam) = :concatenated_data");
...
$count = $stmt->fetchColumn();
        ...
        if ($count > 0) {
            ...
            $sql = "INSERT INTO adressen (woonplaats, postcode, huisnummer) 
                        VALUES (:woonplaats, :postcode, :huisnummer)";
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $sql = "INSERT INTO gebruikers (adres_id, voornaam, achternaam, tussenvoegsel, email, gebruikersnaam, wachtwoord, rol) 
                            VALUES (:adres_id, :voornaam, :achternaam, :tussenvoegsel, :email, :gebruikersnaam, :wachtwoord, :rol)";
                ...
                if ($stmt2->rowCount() > 0) {
                    echo "Address and user data successfully inserted.";
                } else {
